...
class SpectralParam(nn.Module):
    def __init__(self, out_dim, in_dim):
        ...
        with torch.no_grad():
            torch.manual_seed(42)  # Fix random seed during initialization
            linear.weight.copy_(torch.randn(out_dim, in_dim) * (out_dim ** -0.5))

        # Spectral norm + registration
...
class _InducingBase(VariationalMixin):
    ...
    def __init__(self, *args, inducing_rows=None, inducing_cols=None, prior_sd=1., init_lamda=1e-4, learn_lamda=True,
        ...
        self.z_row_rho = nn.Parameter(torch.full((inducing_rows,), init_value), requires_grad=True)

        self.z_col = nn.Parameter(torch.randn(inducing_cols, self._d_in) * inducing_cols ** -0.5)
        self.z_col_rho = nn.Parameter(torch.full((inducing_cols,), inverse_softplus(0.001)))
        ...
        if q_inducing == "full":
            ...
            self._inducing_scale_tril = nn.Parameter(torch.cat([
                torch.full((d,), inverse_softplus(1e-3)), torch.zeros((d * (d - 1)) // 2)
            ]))
            ...
            # Create row_flow and ensure it's on the correct device
    ...
    def sample_u(self):
        ...
        return u.view(self.inducing_rows, self.inducing_cols).to(self.z_row.device)
    ...
    def kl_divergence(self):
        # Get the device where the current module is located
        device = next(self.parameters()).device
        
        if self.inducing_dist is None:
            inducing_kl = torch.tensor(0.0, device=device)
            base_kl = inducing_kl 
        else:
            # Ensure all inputs are on the device where the current module is located
            inducing_dist = self.inducing_dist
            inducing_prior_dist = self.inducing_prior_dist
            
            # Calculate KL divergence and ensure the result is on the current device
            inducing_kl = dist.kl_divergence(inducing_dist, inducing_prior_dist)
            inducing_kl = inducing_kl.to(device)
        ...
        conditional_kl = conditional_kl.to(device)
    ...
    def lamda(self):
        ...
        return F.softplus(self._lamda).clamp(zero, self.max_lamda) if self._lamda is not None else None
    ...
    def inducing_sd(self):
        zero = torch.tensor(0., device=self.device)
        return F.softplus(self._inducing_sd).clamp(zero, self.max_sd_u) if self._inducing_sd is not None else None
    ...
    @property
    def device(self):
        # Try to get device from different attributes, sorted by priority
        if hasattr(self, 'z_row') and self.z_row is not None:
            return self.z_row.device
        elif hasattr(self, 'z_col') and self.z_col is not None:
            return self.z_col.device
        elif hasattr(self, 'inducing_mean') and self.inducing_mean is not None:
            return self.inducing_mean.device
        elif hasattr(self, '_lamda') and self._lamda is not None:
            return self._lamda.device
        else:
            # If all fail, return default cuda:0
...
def register_global_inducing_weights_(module, inducing_rows, inducing_cols, cat_dim=0, **inducing_kwargs):
    ...
    if len(list(_iter_inducing_modules(module))) < 2:
        raise ValueError("'module' must contain at least two inducing layers as the point of this function is to add "
