...
class _InducingBase(VariationalMixin):
    ...
    def __init__(self, *args, inducing_rows=None, inducing_cols=None, prior_sd=1., init_lamda=1e-4, learn_lamda=True,
        ...
        if q_inducing == "full":
            ...
            self.row_flow = MaskedAffineAutoregressiveTransform(
                features=inducing_cols, hidden_features=row_flow_hidden, dropout_probability=0.1)
            ...
            #self.inducing_mean = nn.Parameter(torch.stack([tmp for _ in range(ensemble_size)], dim=0))
            #self.inducing_mean = nn.Parameter(torch.randn(ensemble_size, inducing_rows, inducing_cols) * 0.01)
            self.register_parameter("_inducing_scale_tril", None)
            self.register_parameter("_inducing_row_scale_tril", None)
            self.register_parameter("_inducing_col_scale_tril", None)
            self.register_parameter("_inducing_sd", None)
    ...
    def sample_u(self):
        ...
        return u1.view(self.inducing_rows, self.inducing_cols)
    ...
    def kl_divergence(self):
        ...
        if self.inducing_dist is None:
            ...
            ldj_row = self._u_cache.get("logdet_row",
                          torch.tensor(0., device=self.d_col.device)).sum()
            base_kl = inducing_kl + ldj_row*0.01
        conditional_kl = self.conditional_kl_divergence()
        return base_kl + conditional_kl
