from typing import List, Union

import torch.nn as nn

import torchvision
import timm
import types


def make_network(architecture: str, *args, **kwargs):
    if architecture == "fcn":
        return FCN(**kwargs)
    elif architecture == "cnn":
        return CNN(**kwargs)
    elif architecture == "fcn_resnet50":
        print("Using torchvision.models.segmentation.fcn_resnet50 as segmentation model")
        # 1. Load pre-trained model (default 21 classes)
        net = torchvision.models.segmentation.fcn_resnet50(weights="DEFAULT")
        in_channels = kwargs.get("in_channels", 3)
        out_features = kwargs["out_features"]
        # 2. Replace classification head with custom number of classes
        net.classifier[4] = nn.Conv2d(512, out_features, kernel_size=1)
        # if in_channels != 3:
        #     backbone = net.backbone
        #     conv1 = backbone.conv1
        #     new_conv = nn.Conv2d(in_channels, conv1.out_channels, kernel_size=conv1.kernel_size,
        #                          stride=conv1.stride, padding=conv1.padding, bias=conv1.bias is not None)
        #     backbone.conv1 = new_conv
        
        # # Add feature extraction functionality
        # def get_features(self, x):
        #     input_shape = x.shape[-2:]
        #     features = self.backbone.conv1(x)
        #     features = self.backbone.bn1(features)
        #     features = self.backbone.relu(features)
        #     features = self.backbone.maxpool(features)

        #     features = self.backbone.layer1(features)
        #     features = self.backbone.layer2(features)
        #     features = self.backbone.layer3(features)
        #     features = self.backbone.layer4(features)
            
        #     # Flatten features and return
        #     return features.mean(dim=[2, 3])  # [B, C]
        
        # net.get_features = types.MethodType(get_features, net)
        return net
    elif architecture.startswith("hrnet"):
        print(f"Loading HRNet backbone: {architecture}")
        net = timm.create_model(
            architecture,
            pretrained=True,
            num_classes=kwargs["out_features"],
            in_chans=kwargs.get("in_channels", 3)
        )
        return net
    elif architecture.startswith("resnet"):
        print(architecture)
        net = getattr(torchvision.models, architecture)(num_classes=kwargs["out_features"])
        if "kernel_size" in kwargs:
            kernel_size = kwargs["kernel_size"]
            stride = kwargs.get("stride", 1)
            padding = kwargs.get("padding", kernel_size // 2)
            in_channels = kwargs.get("in_channels", 3)
            bias = net.conv1.bias is not None
            net.conv1 = nn.Conv2d(in_channels, net.conv1.out_channels, kernel_size, stride, padding, bias=bias)
        if kwargs.get("remove_maxpool", False):
            net.maxpool = nn.Identity()
        # net = nn.SyncBatchNorm.convert_sync_batchnorm(net)
        return net
    else:
        raise ValueError("Unrecognized network architecture:", architecture)

def make_network_with_pretrain(architecture: str, *args, **kwargs):
    print("architecture: ", architecture)
    if architecture == "fcn":
        return FCN(**kwargs)
    elif architecture == "cnn":
        return CNN(**kwargs)
    elif architecture.startswith("resnet"):
        print(f"Loading pretrained {architecture} with ImageNet1K_V1 weights")
        # Use official pre-trained model, best version (IMAGENET1K_V1)
        net = getattr(torchvision.models, architecture)(
            weights=torchvision.models.ResNet18_Weights.IMAGENET1K_V1 if architecture == "resnet18" else
                   torchvision.models.ResNet34_Weights.IMAGENET1K_V1 if architecture == "resnet34" else
                   torchvision.models.ResNet50_Weights.IMAGENET1K_V1 if architecture == "resnet50" else
                   torchvision.models.ResNet101_Weights.IMAGENET1K_V1 if architecture == "resnet101" else
                   torchvision.models.ResNet152_Weights.IMAGENET1K_V1 if architecture == "resnet152" else
                   torchvision.models.ResNet18_Weights.IMAGENET1K_V1,  # Default use ResNet18
            num_classes=kwargs["out_features"]
        )
        # net.maxpool = nn.Identity()
        # Don't replace kernel_size, keep original structure of pre-trained model
        # Only handle remove_maxpool parameter
        # if kwargs.get("remove_maxpool", False):
        #     net.maxpool = nn.Identity()
        net = nn.SyncBatchNorm.convert_sync_batchnorm(net)
        print(f"Successfully loaded pretrained {architecture} with {kwargs['out_features']} output classes")
        return net
    else:
        raise ValueError("Unrecognized network architecture:", architecture)



class FCN(nn.Sequential):
    """Basic fully connected network class."""

    def __init__(self, sizes: List[int], nonlinearity: Union[str, type] = "ReLU", bn: bool = False, **layer_kwargs):
        super().__init__()
        nonl_class = getattr(nn, nonlinearity) if isinstance(nonlinearity, str) else nonlinearity

        layer_kwargs.setdefault("bias", not bn)
        for i, (s0, s1) in enumerate(zip(sizes[:-1], sizes[1:])):
            self.add_module(f"Linear{i}", nn.Linear(s0, s1, **layer_kwargs))
            if bn:
                self.add_module(f"BN{i}", nn.BatchNorm1d(s1))
            if i < len(sizes) - 2:
                self.add_module(f"Nonlinarity{i}", nonl_class())


class CNN(nn.Sequential):
    """Basic CNN class with Conv/BN/Nonl/Maxpool blocks followed by a fully connected net. Batchnorm and maxpooling
    are optional and the latter can also only be included after every nth block."""

    def __init__(self, channels: List[int], lin_sizes: List[int], nonlinearity: Union[str, type] = "ReLU",
                 maxpool_freq: int = 1, conv_bn: bool = False, linear_bn: bool = False, kernel_size: int = 3,
                 **conv_kwargs):
        super().__init__()
        nonl_class = getattr(nn, nonlinearity) if isinstance(nonlinearity, str) else nonlinearity
        conv_kwargs.setdefault("bias", not conv_bn)
        for i, (c0, c1) in enumerate(zip(channels[:-1], channels[1:])):
            self.add_module(f"Conv{i}", nn.Conv2d(c0, c1, kernel_size, **conv_kwargs))
            if conv_bn:
                self.add_module(f"ConvBN{i}", nn.BatchNorm2d(c1))
            self.add_module(f"ConvNonlinearity{i}", nonl_class())
            if maxpool_freq and (i + 1) % maxpool_freq == 0:
                self.add_module(f"Maxpool{i//maxpool_freq}", nn.MaxPool2d(2, 2))
        self.add_module("Flatten", nn.Flatten())

        self.add_module("fc", FCN(lin_sizes, nonlinearity=nonlinearity, bn=linear_bn))
